...
export function useQrManagementDetail(
  ...
  async function load() {
    ...
    try {
      ...
      detail.value = result
      try {
        const dataUrl = await QRCode.toDataURL(buildQrTraceUrl(result.code), {
          width: 256,
          margin: 2,
        ...
        if (!current.signal.aborted)
          qrError.value =
            cause instanceof Error ? cause.message : '二维码图片加载失败'
      }
    } catch (cause) {
      if (!current.signal.aborted)
  ...
  watch(
    [id, activeTab, scanPage, scanRetry],
    async ([qrId, tab], _old, onCleanup) => {
      scanData.value = null
      scanError.value = ''
      ...
      try {
        ...
        if (!current.signal.aborted)
          scanError.value =
            cause instanceof Error ? cause.message : '扫码记录加载失败'
      } finally {
  ...
  function toggleAvailability() {
    ...
    confirmAction({
      ...
      content: item.disabled
        ? `恢复 ${item.code} 后可继续扫码查询。`
        : `停用 ${item.code} 后扫码不可用，已有绑定和扫码记录会保留，可随时恢复。`,
  ...
  async function printLabel() {
    ...
    try {
      ...
      doc.open()
      doc.write(
        '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>二维码标签</title></head><body></body></html>',
      )
      doc.close()
      const style = doc.createElement('style')
      style.textContent =
        '@page{size:60mm 80mm;margin:4mm}body{margin:0;text-align:center;font:12px sans-serif;color:#111}img{display:block;width:42mm;height:42mm;margin:2mm auto}p{margin:2mm 0;overflow-wrap:anywhere}strong{font-size:13px}'
    ...
    return [
      {
        ...
        fields: [
          ...
          ['产品', item.productName],
          ['款式', item.style],
          ['尺码', item.productSizes?.join('、').toUpperCase()],
          ['颜色', item.color],
          ['执行标准', item.executionStandard],
          ['洗涤说明', item.washingInstructions],
          ['安全类别', item.safetyCategory],
          ['生产批次', item.productionBatch],
          ['生产日期', item.productionDate],
          ['生产工厂', item.factoryName],
        ],
      },
      {
        title: '生产单位信息',
      ...
      },
      {
        title: '检测报告',
        fields: [
          ['报告编号', item.qualityReportNo],
          ['检测日期', item.qualityInspectionDate],
        ],
      },
      {
        title: '学生信息',
      ...
      },
      {
        ...
        fields: [
          ['家长', item.parentName],
          ['关系', item.parentRelation],
          ['手机号', item.phoneMasked],
        ],
      },
      {
        title: '二维码数据',
        fields: [
          ...
          ['绑定时间', item.studentBoundAt],
          ['扫码次数', String(item.scanCount)],
          ['首次扫码', item.firstScannedAt],
          ['最后扫码', item.lastScannedAt],
        ],
      },
    ] as Array<{
      title: string
