...
export function getFiles(params: Record<string, string | number>) {
  return request<ResourcePage<ManagedFile>>(
    `/api/v1/system/files?${queryString(params)}`,
  )
}
...
function remove(file: ManagedFile) {
  confirmAction({
    title: '确认删除文件',
    content: `删除后无法恢复，确定删除“${file.name}”吗？`,
    okType: 'danger',
    async onOk() {
...
<template>
  <section class="mx-auto max-w-400 space-y-4">
    <div class="page-card">
      <PageHeader title="文件管理" description="集中管理平台图片和检测报告">
        <template #actions>
