...
const itemFields = `q.id, q.code, q.status AS qrStatus, q.disabled,
  ...
  p.production_unit_credit_code AS productionUnitCreditCode,
...
class QrManagementService extends Service {
  ...
  async list(query) {
    ...
    const [items, counts, totals, scans, schools] = await Promise.all([
      ...
      this.select(
        'SELECT COUNT(*) AS total FROM qr_scan_records WHERE scanned_at >= :start AND scanned_at < :end',
        { start, end },
      ),
      this.select(
        'SELECT DISTINCT school_name AS name FROM qr_student_bindings ORDER BY school_name',
      ),
    ])
    return {
      ...
      statistics: {
        ...
        bound: Number(totals[0].bound),
        activated: Number(totals[0].activated),
        todayScans: Number(scans[0].total),
      },
  ...
  async detail(id) {
    const [item] = await this.select(
      ...
      report.id AS qualityReportId, report.report_no AS qualityReportNo,
      report.inspection_date AS qualityInspectionDate,
      ...
      LEFT JOIN quality_reports report ON report.id = (
        SELECT r.id FROM quality_reports r
        JOIN sys_files f ON f.id = r.file_id
    ...
    const result = {
      ...itemJson(item),
      hasQualityReport: Boolean(item.hasQualityReport),
      qualityReportId:
        item.qualityReportId == null ? null : Number(item.qualityReportId),
    }
    for (const key of [
      'generatedAt',
      'studentBoundAt',
  ...
  async setDisabled(id, value) {
    return this.app.model.transaction(async (transaction) => {
      const qr = await this.app.model.QrCode.findByPk(id, {
        transaction,
        lock: transaction.LOCK.UPDATE,
      })
      if (!qr) fail('二维码不存在', 404)
      if (qr.status === 'voided') fail('已作废二维码不能停用或恢复')
      if (Boolean(qr.disabled) !== value.expectedDisabled)
        fail('二维码状态已变更，请刷新后重试', 409)
      if (Boolean(qr.disabled) !== value.disabled) {
        ...
        await this.ctx.service.qrcodes.log(
          value.disabled ? '停用二维码' : '恢复二维码',
          'qr_code',
          id,
          { code: qr.code, disabled: value.disabled },
          transaction,
        )
      }
  ...
  async save(id, value) {
    return this.app.model.transaction(async (transaction) => {
      ...
      const existing = await this.app.model.QrStudentBinding.findByPk(id, {
        transaction,
      })
      const permission = existing ? 'edit' : 'create'
      if (
        !(await this.ctx.service.auth.hasPermission(
  ...
  async scans(id, query) {
    if (!(await this.app.model.QrCode.findByPk(id, { attributes: ['id'] })))
      fail('二维码不存在', 404)
    const { page, pageSize, offset } = this.ctx.helper.pagination(query)
    const chinaDay = new Date(Date.now() + 8 * 3600000)
      .toISOString()
    ...
    const [items, counts] = await Promise.all([
      this.select(
        `SELECT r.id, r.code, r.studentName, r.scannedAt, ${classify} AS scanType ${baseSql}
        ORDER BY r.scannedAt DESC, r.id DESC LIMIT :limit OFFSET :offset`,
